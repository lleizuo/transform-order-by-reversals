...
def skeleton():
    ...
    for work_list in rand_select:
        work_list = [ str(int(num)*givesign()) for num in work_list]
        print(work_list)
        start_time = time.time()
        result = subprocess.run(["python3","signed.py"] + work_list,capture_output=True).stdout
        end_time = time.time()
    ...
    pd.DataFrame(data_list).to_csv("../data2/"+str(n)+"_signed.csv")
